...
async function requestWakeLock() {
    ...
    catch (err) { console.warn("Wake Lock err:", err); }
}
...
window.toggleKeepAwake = async function() {
    ...
    if (wakeLock === null) {
        ...
        btn.style.borderColor = 'rgba(16,185,129,0.4)'; btn.style.color = '#34d399';
        btn.querySelector('.lbl').innerHTML = 'Awake<br>ON';
    } else {
        releaseWakeLock();
        btn.style.borderColor = 'rgba(239,68,68,0.4)'; btn.style.color = '#f87171';
        btn.querySelector('.lbl').innerHTML = 'Awake<br>OFF';
    }
}
...
function syncDecksToLibrary() { 
    ...
    state.deckData.forEach((d, i) => { 
        ...
        if (d.name && d.name !== "") {
            let existing = allE.find(ex => ex.name && ex.name.toLowerCase() === d.name.toLowerCase());
            if (!existing) { 
                let newId = generateDeckID();
                d.id = newId;
                state.savedDecks.push({ id: newId, name: d.name, colors: [...d.colors] }); 
                allE.push({ id: newId, name: d.name, colors: [...d.colors] }); nw = true; 
            } else { d.id = existing.id; }
        }
    }); 
    if (nw) saveData(); 
}
...
function goToPlayers() { 
    ...
    let validD = state.deckData.map((d, i) => ({ ...d, origIdx: i })).filter(d => d.name && d.name.trim() !== "");
    ...
    for (let i = 0; i < state.players; i++) { 
        ...
        let hasLock = lockVal !== -1; let atBanLimit = state.playerBans[i].length >= maxBans;
...
function renderSavedPlayers() { 
    ...
    c.innerHTML = state.savedPlayers.map((p, i) => {
        ...
        return `<div class="relative flex flex-col items-center shrink-0 mt-2 group"><button onclick="window.deleteSavedPlayer(${i})" class="absolute -top-1 -right-1 bg-app-surface text-slate-500 text-[9px] size-[18px] flex items-center justify-center rounded-full border border-white/10 shadow-sm z-10">✕</button><div onclick="window.quickAdd('${esc(pName)}')" class="setup-input size-12 rounded-full border border-app-primary bg-app-surface-light flex items-center justify-center font-bold text-white cursor-pointer">${esc(initial)}</div><span class="text-[9px] truncate w-14 text-center mt-1 text-slate-400 uppercase font-bold">${esc(pName)}</span></div>`;
    }).join(''); 
}
...
async function deleteSavedPlayer(i) { const c = await mfModal.show("Remove Player?", `Remove from roster?`, "person_remove", "confirm"); if (c) { state.savedPlayers.splice(i, 1); saveData(); renderSavedPlayers(); } }
...
function executeAssignment() { 
    let validD = state.deckData.filter(d => d.name && d.name.trim() !== ""); 
...
function initBattlefield() { 
    playTransition(GIFS.BATTLE, 2600, async () => { 
        renderBattlefield(); startClock(); switchScreen(5); 
    }); 
}
...
window.undoLastAction = function() {
    ...
    state.currentMatch = state.undoStack.pop(); saveData(); renderBattlefield();
}
...
function handleTapStart(e, idx, amt) {
    ...
    saveUndoState(); clearTimeout(holdTimer); clearInterval(holdInterval); changeLife(idx, amt);
...
function renderBattlefield() { 
    ...
    state.currentMatch.forEach((p, i) => { 
        ...
        if (count === 4 && state.layoutMode === 'cross') { 
            if (i === 0) { pos = 'cross-pos-0'; rotDeg = 180 } if (i === 1) { pos = 'cross-pos-1'; rotDeg = 90 } 
            if (i === 2) { pos = 'cross-pos-2'; rotDeg = -90 } if (i === 3) { pos = 'cross-pos-3'; rotDeg = 0 } 
        } else { 
            if (count === 2 && i === 0) rotDeg = 180; if (count === 3 && i === 0) { rotDeg = 180; pos = 'bf-3p-top' } 
            if (count >= 4 && (i === 0 || i === 1)) rotDeg = 180; if (count === 6 && i === 2) rotDeg = 180; 
        } 
...
async function checkEliminations() { 
    ...
    for (let i = 0; i < state.currentMatch.length; i++) { 
        ...
        if (!p.isDead) { 
            let cmdrDeath = false; if (p.cmdrDmg) { cmdrDeath = Object.values(p.cmdrDmg).some(d => d >= 21); } 
...
window.toggleCenterMenu = function() { 
    ...
    if (menu) {
        if(menuOpen) { 
            ...
            icon.classList.add('material-symbols-outlined', 'text-3xl', 'rotate-90');
            ...
            const R = 95; const startAngle = -Math.PI / 2; const angleStep = (Math.PI * 2) / btns.length;
            btns.forEach((btn, index) => {
                const angle = startAngle + index * angleStep;
                const x = Math.round(Math.cos(angle) * R); const y = Math.round(Math.sin(angle) * R);
                btn.style.transform = `translate(${x}px, ${y}px) scale(1)`; btn.style.opacity = '1'; btn.style.pointerEvents = 'auto'; btn.style.transitionDelay = `${index * 0.05}s`;
            });
        } else { 
            ...
            btns.forEach(btn => { btn.style.transform = `translate(0px, 0px) scale(0.3)`; btn.style.opacity = '0'; btn.style.pointerEvents = 'none'; btn.style.transitionDelay = '0s'; });
        }
    }
}
...
function renderHistory() { 
    ...
    c.innerHTML = state.history.slice(0, 5).map((m, i) => {
        ...
        let dateStr = isNaN(dateObj) ? 'Date Error' : dateObj.toLocaleDateString() + ' ' + dateObj.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        let winnerObj = m.participants ? m.participants.find(p => p.result === 'Winner') : null;
        let winnerName = winnerObj ? winnerObj.snapshotName : (m.winner || 'Unknown');
        let durationHTML = m.durationSecs ? `<span class="text-[9px] text-slate-500 flex items-center gap-0.5"><span class="material-symbols-outlined text-[10px]">timer</span> ${formatTime(m.durationSecs)}</span>` : '';
        
        let pList = m.participants ? m.participants.map(p => `<div class="flex justify-between py-0.5 ${p.result === 'Winner' ? 'text-yellow-400 font-bold' : 'text-slate-300'}"><span class="truncate">${esc(p.snapshotName)}</span><span class="text-slate-400 font-medium truncate ml-2 text-right w-1/2">${esc(p.snapshotDeckName)}</span></div>`).join('') : '';
...
export function handleCommanderNext() {
    if (state.step === 1) { state.playerLocks = []; state.playerBans = []; goToDecks(); } 
        ...
        let hasErrors = false;
        state.deckData.forEach((d, i) => { 
            // FIX: Protegemos contra nulos en caso de recarga a la fuerza
        ...
        let validDecks = state.deckData.filter(d => d.name && d.name.trim() !== "");
        if (validDecks.length < state.players) return mfModal.show("Warning", `Need more physical decks!`, "warning"); 
        
        savePlayerInputs(); let nwP = false; 
        for (let i = 0; i < state.players; i++) { 
            let v = state.tempPlayerNames[i] || 'Player ' + (i + 1); state.tempPlayerNames[i] = v; 
            let existing = state.savedPlayers.find(p => p.name && p.name.toLowerCase() === v.toLowerCase());
