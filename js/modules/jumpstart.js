...
function startJSTimer() {
    if(!state.js.startTime) state.js.startTime = Date.now();
    if(!state.matchDurationSeconds) state.matchDurationSeconds = 0;
    if(jsInterval) clearInterval(jsInterval);
    jsInterval = setInterval(() => { if(!state.matchFinished) state.matchDurationSeconds++; }, 1000);
}
...
export function handleJumpstartNext() {
    if(state.step === 7) goToJSPlayers();
        ...
        state.tempPlayerNames.forEach(pN => {
            if(!state.savedPlayers.some(p => p.name.toLowerCase() === pN.toLowerCase())) {
...
export function goBackJumpstart() { if(state.step === 8) switchScreen(7); if(state.step === 9) switchScreen(8); }
...
function generateJSSwiss() {
    ...
    state.js.players = names.map(n => ({ id: getFoxIdByName(n), name: n, points: 0, played: [], defeated: [] }));
...
function renderJSSwiss() {
    const tabs = document.getElementById('js-tabs'); const mCont = document.getElementById('js-matches'); if(!tabs || !mCont) return;
    ...
    if (state.js.currentView === 'standings') {
        let sorted = [...state.js.players].sort((a,b) => {
            ...
            let aDefeatedB = a.defeated && a.defeated.includes(b.name); let bDefeatedA = b.defeated && b.defeated.includes(a.name);
...
function setJSWinner(r, m, winnerName) {
    ...
    let w = state.js.players.find(p => p.name === winnerName); let loserName = match.p1 === winnerName ? match.p2 : match.p1;
    if(!w.defeated) w.defeated = []; w.defeated.push(loserName); w.points += 3;
    saveData(); closeJSModal(); renderJSSwiss();
}

function revertJSSwissWinner() {
    let matches = state.js.rounds[state.js.currentRound]; let lastMatch = [...matches].reverse().find(m => m.winner !== null);
    ...
    p1.played.pop(); p2.played.pop(); lastMatch.winner = null; saveData(); renderJSSwiss();
}
...
function showJSUltimateWinner(sortedPlayers) {
    ...
    playTransition(GIFS.WINNER, 3200, () => {
        state.matchFinished = true; let w = sortedPlayers[0];
...
window.setJSPlayers = setJSPlayers; window.setJSView = setJSView; window.openJSMatchModal = openJSMatchModal; window.toggleJSReady = toggleJSReady; window.closeJSModal = closeJSModal; window.setJSWinner = setJSWinner; window.revertJSSwissWinner = revertJSSwissWinner; window.finishSwiss = finishSwiss; window.quickAddJS = quickAddJS; window.generateNextSwissRound = generateNextSwissRound;